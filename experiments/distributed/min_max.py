...
benign_data = pd.concat([pd.read_csv(os.path.join('../../data/UCI-MLR', device, 'benign_traffic.csv'))[:5000] for device in device_list])
np.savetxt('min.txt', np.array(benign_data).min(axis=0))
np.savetxt('max.txt', np.array(benign_data).max(axis=0))
